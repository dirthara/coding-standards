...
title: Documentation
sidebar_position: 7
description: What the README owns, what the docs directory owns, and the conventions the documentation site depends on.
---

Documentation lives in two places, with no overlap between them. The root markdown files describe the *repository*. The
`docs` directory describes the *library*.
...
`docs` holds the usage documentation as markdown. The `dirthara/docs` repository pulls these files at the ref each
version pins and builds
<https://dirthara.github.io/docs/> from them with Docusaurus, so write them as
if they are already on that site. The package never publishes a site of its own; `README.md` and `composer.json`
(`homepage`, `support.docs`) both point at the shared one.

The conventions the build depends on:

- **Front matter on every page**, carrying `id`, `title`, `sidebar_position`, and `description`. Add `sidebar_label`
...
Keep it truthful against the source. When behaviour changes, the page that describes it changes in the same commit — a
pull request that changes behaviour and not its documentation is incomplete, and review should say so.
